...
assert_contains "skills/executing-plans/SKILL.md" 'Record each append-only `## Execution Log` entry with a local-time timestamp with minute precision, then the completed task id, next task id, last command run, last validation result, and any blocker.'
...
assert_contains "skills/subagent-driven-development/SKILL.md" 'Record each append-only `## Execution Log` entry with a local-time timestamp with minute precision, then the completed task id, next task id, last command run, last validation result, and any blocker.'
...
assert_contains "skills/writing-plans/SKILL.md" "Each appended execution-log entry should start with a timestamp in local timezone with minute precision"
assert_contains "skills/writing-plans/SKILL.md" 'In `## Execution Log`, append entries only. Each entry should start with a local-time timestamp with minute precision'
